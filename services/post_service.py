...
from services.ai_content import MONTHS_RU, WEEKDAYS_RU, generate_post_content
...
MULTIPOST_THRESHOLD = 4096  # Characters threshold for splitting (Telegram's max)
MULTIPOST_TARGET_LENGTH = 3500  # Target length per part
...
def split_post_into_parts(
    text: str, max_length: int = MULTIPOST_TARGET_LENGTH
) -> list[str]:
    """
...
def format_post_text(
    ...
    if (
        len(post_text) > max_length
        and settings.text_template != TextTemplate.LONG.value
    ):
...
async def generate_post_data(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            recipe = content.get("recipe", {})
            image_prompt = recipe.get(
                "image_prompt_en", recipe.get("name", "healthy food")
            )

            image_bytes = await generate_food_image(
                recipe_name=recipe.get("name", "Recipe"), english_prompt=image_prompt
            )
            ...
            logger.error(
                f"Post data generation attempt {attempt} failed: {e}", exc_info=True
            )
...
async def send_preview_to_admin(
    ...
    try:
        ...
        if is_multipost:
            ...
            # Truncate if too long for caption (max 4096 chars)
        ...
        logger.info(
            f"Preview sent to admin {mask_user_id(admin_id, config.debug_mode)}"
        )
        return True
...
async def publish_pending_post(bot: Bot, post_id: str, channel_id: str) -> bool:
    """
    ...
    try:
        ...
        if is_multipost and parts:
            ...
            for i, part in enumerate(parts):
                ...
                if is_first and image_bytes:
                    ...
                    await bot.send_photo(
                        chat_id=channel_id, photo=photo, caption=part, parse_mode="HTML"
                    )
                else:
                    await bot.send_message(
        ...
        logger.info(
            f"Published pending post {post_id} to channel {mask_channel_id(channel_id, config.debug_mode)}"
        )
        return True
...
async def post_to_channel(
    ...
    try:
        ...
        if image_bytes:
            ...
            photo = BufferedInputFile(image_bytes, filename="recipe.jpg")
            await bot.send_photo(
                chat_id=channel_id, photo=photo, caption=post_text, parse_mode="HTML"
            )
        else:
